...
ENV_MW_PROJECT_NAME = "MW_PROJECT_NAME"
ENV_MW_GRPC_PORT = "MW_GRPC_PORT"

GRPC_PORT = 9319
"""The default network port for OTLP/gRPC is 9319.
See https://opentelemetry.io/docs/specs/otlp/#otlpgrpc-default-port

Note: gRPC transport is not supported by middleware-llmobs. This is retained only for the
unsupported gRPC code path."""
...
def get_env_collector_endpoint() -> Optional[str]:
    ...
    Reads the standard OpenTelemetry OTLP endpoint environment variable.

    Returns:
        Optional[str]: The collector endpoint URL if found, None otherwise.
    """
    return os.getenv(ENV_OTEL_EXPORTER_OTLP_ENDPOINT)


def get_env_service_name() -> str:
    """
    Get the service name from environment variables.

    Returns:
        str: The service name from OTEL_SERVICE_NAME, defaults to "default".
    """
    return os.getenv(ENV_OTEL_SERVICE_NAME, DEFAULT_SERVICE_NAME)
...
def get_env_project_name() -> Optional[str]:
    ...
    Returns only the explicitly configured MW_PROJECT_NAME; callers are responsible for falling
    back to the service name when this is None.

    Returns:
        Optional[str]: The project name from MW_PROJECT_NAME, or None if unset.
    """
    return os.getenv(ENV_MW_PROJECT_NAME)
...
def get_env_client_headers() -> Optional[Dict[str, str]]:
    ...
    Parses the OTEL_EXPORTER_OTLP_HEADERS environment variable into a dictionary
    of HTTP headers using the W3C Baggage HTTP header format. This is where the
    Middleware ``Authorization`` key is supplied, e.g.::
    ...
    """
    if headers_str := os.getenv(ENV_OTEL_EXPORTER_OTLP_HEADERS):
...
def get_env_grpc_port() -> int:
    ...
    Note:
        gRPC transport is not supported by middleware-llmobs; this exists only for the
        retained (and disabled) gRPC code path.

    Returns:
        int: The gRPC port number.

    Raises:
        ValueError: If MW_GRPC_PORT is set but not a valid integer.
    """
    if not (port := os.getenv(ENV_MW_GRPC_PORT)):
        return GRPC_PORT
    if port.isnumeric():
        return int(port)
    raise ValueError(
        f"Invalid value for environment variable {ENV_MW_GRPC_PORT}: "
